...
export interface UncoveredFn { display: string; line: number }
...
export function TestsModal({ lineNum, tests, totalTests, onSelect, onClose }: {
  lineNum: number
  tests: TestRef[]
  totalTests: number
  ...
  return (
    ...
    >
      <div
        ...
        style={{ background: '#16161d', border: '1px solid #2a2a36', borderRadius: 12, width: 420, maxWidth: '90vw', maxHeight: '60vh', display: 'flex', flexDirection: 'column', boxShadow: '0 24px 64px rgba(0,0,0,0.7)' }}
      >
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #2a2a36', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#c4c4d4' }}>Tests covering line {lineNum}</span>
            <span style={{ fontSize: 11, color: tests.length < totalTests ? '#f59e0b' : '#4b4b60', marginLeft: 8 }}>
              {tests.length}{totalTests > 0 ? ` of ${totalTests}` : ''} test{tests.length !== 1 ? 's' : ''}
              {tests.length < totalTests && ' — partial coverage'}
            </span>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#4b4b60', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '0 2px' }}>×</button>
        </div>
        <div style={{ overflowY: 'auto', padding: '8px 0' }}>
          {tests.map(ref => (
            <button
              key={ref.testId}
              onClick={() => { onSelect(ref.suiteId, ref.testId); onClose() }}
              style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: '10px 18px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}
              onMouseEnter={e => (e.currentTarget.style.background = '#1e1e2e')}
              onMouseLeave={e => (e.currentTarget.style.background = 'none')}
            >
              <span style={{ fontSize: 11, color: '#6b7280', marginBottom: 2 }}>{ref.suiteName}</span>
        ...
        <div style={{ padding: '10px 18px', borderTop: '1px solid #2a2a36', fontSize: 11, color: '#3a3a4e' }}>
