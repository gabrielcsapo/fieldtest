      ...
      >
        ...
        {suite.duration !== undefined &&
          ...
          suite.status !== "running" && (
            <span
              style={{ fontSize: 10, color: "#3a3a4e", flexShrink: 0, fontFamily: "monospace" }}
            >
...
export function TestTree({
  ...
  const filteredSuites = state.suites
    .map((suite) => ({
      ...suite,
      tests: suite.tests.filter(
        (t) => !q || t.name.toLowerCase().includes(q) || suite.name.toLowerCase().includes(q),
      ),
    }))
  ...
  return (
    ...
    >
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: "auto", padding: "4px 0" }}>
        ...
        {items.length === 0 && q && (
          <div
            style={{ padding: "20px 16px", fontSize: 12, color: "#4b4b60", textAlign: "center" }}
          >
        ...
        {sortedSuites.length > 0 && !state.running && !state.lastRunAt && (
          ...
          >
            ...
            {" — "}click ▶ to run all or ▶ on a suite.
            <br />
                ...
                >
                  {item.kind === "suite" ? (
                    ...
                    <TestRow
                      ...
                      onRun={(e) => {
                        e.stopPropagation();
                        onRunTest(item.suite.id, item.test.id);
                      }}
                      disabled={state.running}
                    />
                  )}
