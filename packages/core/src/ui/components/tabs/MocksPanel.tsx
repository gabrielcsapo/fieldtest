...
function CallRow({ call, index }: { call: MockCall; index: number }) {
  ...
  return (
    <div style={{ borderBottom: '1px solid #1a1a24' }}>
      ...
      >
        <span style={{ color: '#3a3a4e', fontSize: 10, fontFamily: 'monospace', flexShrink: 0, width: 20 }}>
          {index}
        </span>
        <code style={{ fontSize: 11, color: '#c4c4d4', fontFamily: 'monospace', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          <ArgList args={call.args} />
          <span style={{ color: '#4b4b60', margin: '0 6px' }}>→</span>
          {call.threw
            ? <span style={{ color: '#f87171' }}>threw {resultDisplay}</span>
            : <span style={{ color: '#34d399' }}>{isMultiline && !expanded ? '{…}' : resultDisplay}</span>}
        </code>
        <span style={{ fontSize: 10, color: '#3a3a4e', fontFamily: 'monospace', flexShrink: 0 }}>{time}</span>
        {isMultiline && (
          <span style={{ fontSize: 10, color: '#4b4b60', flexShrink: 0 }}>{expanded ? '▲' : '▼'}</span>
        )}
      </button>
      {expanded && isMultiline && (
        <pre style={{
          margin: 0, padding: '6px 12px 6px 40px',
          fontSize: 11, fontFamily: 'monospace', color: '#34d399',
          background: '#0f0f13', whiteSpace: 'pre-wrap', wordBreak: 'break-all',
        }}>
          {resultDisplay}
        </pre>
      )}
...
function MockEntryRow({ entry }: { entry: MockEntry }) {
  ...
  return (
    <div style={{ marginBottom: 8, border: '1px solid #2a2a36', borderRadius: 8, overflow: 'hidden' }}>
      {/* Header */}
      <button
        onClick={() => setOpen(v => !v)}
        style={{
          width: '100%', display: 'flex', alignItems: 'center', gap: 10,
          padding: '8px 12px', background: '#1a1a24', border: 'none',
          cursor: 'pointer', textAlign: 'left',
        }}
      >
        <span style={{
            ...
            Array.from(byFn.entries()).map(([fnName, calls]) => (
              <div key={fnName}>
                <div style={{
                  ...
                  color: '#818cf8', background: '#13131a',
                  borderTop: '1px solid #2a2a36', borderBottom: '1px solid #1a1a24',
    ...
    return (
      <div style={{ padding: '32px 20px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 12, color: '#3a3a4e', marginTop: 6 }}>
          Use <code style={{ fontFamily: 'monospace', color: '#818cf8' }}>mock(moduleId, factory)</code> at the top of your test file
        </div>
      </div>
    )
  }
