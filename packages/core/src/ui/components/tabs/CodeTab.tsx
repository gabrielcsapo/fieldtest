...
function FileView({
  ...
  const handleLineClick = useCallback(
    (lineNum: number, tests: TestRef[]) => setModal({ lineNum, tests }),
    [],
  );
  ...
  return (
    <div>
      ...
      >
        {[
          {
            color: hasTestCov ? "rgba(99,102,241,0.35)" : "rgba(34,197,94,0.25)",
            label: hasTestCov ? "Touched by this test" : "Covered",
          },
          { color: "rgba(239,68,68,0.25)", label: "Not covered" },
          ...(totalTestsForFile > 0
            ? [{ color: "rgba(245,158,11,0.3)", label: "Partial coverage (click)" }]
            : []),
        ].map(({ color, label }) => (
          <div
            key={label}
      ...
      <div style={{ fontFamily: "monospace", fontSize: 12, overflowX: "auto" }}>
        {lines.map((text, i) => {
          ...
          const isPartial =
            allTests.length > 0 && totalTestsForFile > 0 && allTests.length < totalTestsForFile;
            ...
            >
              ...
              {clickable && (
                ...
                >
                  ...
                  >
                    {totalTestsForFile > 0
                      ? `${allTests.length}/${totalTestsForFile}`
                      : allTests.length}
                  </span>
                </div>
              )}
...
export function CodeTab({ suiteName, coverage, testCoverage, suites, onSelectTest }: Props) {
  ...
  useEffect(() => {
    ...
    const match =
      findFileForSuite(suiteName, pool) ??
      (testCoverage
        ? (Object.keys(testCoverage).find((p) => !p.includes(".test.") && !p.includes(".spec.")) ??
          null)
        : null);
  ...
  const sortedFiles = filePool.sort(
    (a, b) => coveragePct(coverage[a] ?? coverage[b]) - coveragePct(coverage[b] ?? coverage[a]),
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      {error && (
        <div
          style={{ padding: "16px 20px", color: "#fca5a5", fontSize: 12, fontFamily: "monospace" }}
        >
