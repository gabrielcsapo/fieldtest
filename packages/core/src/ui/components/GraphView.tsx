  ...
  const graphData = useMemo(() => {
    ...
    for (const mod of raw.nodes) {
      ...
      if (fileCov) {
        ...
        if (stmts.length > 0)
          coveragePct = Math.round(
            (stmts.filter((n) => (n as number) > 0).length / stmts.length) * 100,
          );
      }
      ...
      nodes.push({
        id: mod.id,
        type: "file",
        label: parts[parts.length - 1] ?? mod.shortPath,
        cluster,
        x: 0,
        y: 0,
        z: 0,
...
function EdgeLines({
  ...
  const { importPos, importCol, covPos, covCol } = useMemo(() => {
    ...
    return {
      importPos: new Float32Array(importPos),
...
function Scene({
  ...
  return (
    <>
      ...
      <mesh position={[0, 0, 0]} onClick={() => onSelect(null)} visible={false}>
...
function InfoPopover({
  ...
  const color =
    node.type === "suite"
      ...
      : node.coveragePct != null
        ? coverageColor(node.coveragePct)
        : "#3b82f6";
      ...
      <div style={{ maxHeight: "calc(100vh - 140px)", overflowY: "auto" }}>
        ...
        {node.type === "suite" && (
          ...
          >
            ...
            {(node.failCount ?? 0) > 0 && (
              <Stat label="Fail" value={String(node.failCount)} color="#ef4444" />
            )}
          </div>
        )}
          ...
          <Section title={`Tested by (${coveredBy.length})`} color="#818cf8">
            {coveredBy.map((n) => (
              <NodeChip
                key={n.id}
                node={n}
                onClick={n.suiteId ? () => onSelectSuite(n.suiteId!) : undefined}
              />
            ))}
          </Section>
        )}
