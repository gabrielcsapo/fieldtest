...
function recordCall(moduleId: string, fnName: string, args: unknown[], result: unknown, threw: boolean) {
...
function wrapWithSpies(moduleId: string, exports: Record<string, unknown>): Record<string, unknown> {
    ...
    wrapped[key] = (...args: unknown[]) => {
      ...
      try {
        ...
        if (result instanceof Promise) {
          return result.then(
            (resolved) => { recordCall(moduleId, key, args, resolved, false); return resolved },
            (err) => { recordCall(moduleId, key, args, err, true); throw err },
          )
        }
      } catch (err) {
        result = err
  ...
  return Array.from(_registry.keys())
    ...
    .map(moduleId => ({
      ...
      calls: _callLog
        ...
        .map(({ fnName, args, result, threw, timestamp }) => ({ fnName, args, result, threw, timestamp })),
    }))
}
