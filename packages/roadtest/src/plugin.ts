...
interface RoadtestOptions {
  ...
  include?: string;
  /**
   * Inject the roadtest entry script into index.html via transformIndexHtml.
...
const NODE_ENV_OVERRIDE_RE = /^\s*\/\/\s*@roadtest-env\s+node\s*$/m;
 ...
 * is a Node built-in. node_modules are skipped — use `// @roadtest-env node`
...
/**
 * ID for the virtual module that replaces `roadtest` when a test file is
...
export const render = () => { throw new Error('[roadtest] render() is not available in node tests') }
...
async function transformTestFile(
  ...
  for (const node of allImports) {
    ...
    if (node.source.value === runtimePkg || node.source.value === "roadtest")
...
function detectRuntimePkg(root: string): string {
  try {
    ...
    if ("roadtest" in all) return "roadtest";
  } catch {
